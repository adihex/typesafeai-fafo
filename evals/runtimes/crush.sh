...
rt_fake_home() {
  ...
  mkdir -p "$h/.local/share/crush"
  for f in providers.json hyper.json projects.json; do
    ln -s "$REAL_HOME/.local/share/crush/$f" "$h/.local/share/crush/" 2>/dev/null
  done
...
rt_invoke() {
  (
    cd "$2" || exit 1
    HOME="$4" PATH="$REAL_HOME/.local/bin:$REAL_HOME/bin:$PATH" TYPESAFE_API_KEY="${TYPESAFE_API_KEY:-}" crush run \
      "$(cat "$1")" >"$3" 2>"$3.stderr"
  )
}
